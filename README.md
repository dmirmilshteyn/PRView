...
Inline comment composers and saved code threads have a **Send to chat** button. Collect comments from multiple line ranges as attachments, inspect or remove them in the chat composer, then add a question and send the batch to Luna. Draft comments sent this way are not saved as PR comments. Sent attachments retain their code excerpts, before/after side, and revision in the conversation history. Pending attachments remain available while switching Code/Tour tabs or collapsing chat, until you leave the PR page.
