...
The image includes Bun, Python, Git, uv and the GitHub CLI. To rebuild and restart the same development image with Docker Compose, run `docker compose up -d --build` from the checkout. Set `PRVIEW_PORT` to override the default host port if 9999 is already occupied. Run project commands inside the container (`docker compose exec prview …`) or a devcontainer terminal.
...
The devcontainer binds the existing host `~/.config/gh` directory read-only at `/run/gh-auth` and sets `GH_CONFIG_DIR` to that mount. No credentials are exported or copied into the workspace or a PRView-specific directory. Authenticate `gh` on the host first. A macOS Keychain-only credential is not exposed by a filesystem bind; the container needs a token already stored in that existing config or a `GH_TOKEN` supplied through your local container environment. PRView does not rewrite your host authentication. Both container configurations pass through an optional host `GH_TOKEN`. For a Keychain-backed login with Compose, launch from the host with `GH_TOKEN="$(gh auth token)" docker compose up -d --build`; this uses the existing host login without writing a token file.
...
Both `track` and `sync` require an explicit repository. Tracking uses the same import as targeted sync and preserves other imported PRs. These commands register and select the imported repository. `repo track` verifies, clones and registers a repository without importing PRs; `repo use` selects an already registered repository without contacting GitHub.

The dashboard has a **Sync repository** button beside the picker. It updates the local clone and incrementally imports PRs, displaying progress and completed results while continuing after individual failures. Each PR has a ten-minute limit; there is no overall sync deadline. Retry keeps completed snapshots and reuses unchanged code while refreshing reviews, checks and discussion. Closed PRs are removed from the open queue unless retained as native stack context. The dashboard also has a **Track PR** button for importing a PR by number and a repository switcher with **Add repository…**. Switching reads cached imports and never contacts GitHub or starts a sync. The selection is saved for this local workspace.
...
Sync includes the complete Markdown description, review requests and decisions, checks with detail links, issue discussion, and inline GitHub threads with replies and resolution state. It also reads changed files from the repository volume at the diff's merge base and head commit for expanded context and full-file viewing. Binary, oversized and unavailable files retain an explicit context-unavailable message. A PR changing during import fails that PR only; its previous published snapshot remains available. Each published `snapshot.json` contains metadata and code together and is replaced atomically.
...
## Repository storage and rendering

Both Compose and the devcontainer mount the named volume `prview-repository-data` at `/var/lib/prview/repositories`. Each `owner/repo` has its own clone. Sync fetches repository branches and PR head refs; reviews and tours run in that clone and inspect reference source with `git show` at the supplied commit. The shared working tree follows the default branch. `PRVIEW_REPOSITORIES_DIR` overrides the volume path for deployments and tests. Rebuild/recreate the devcontainer to apply the new mount and Git installation. Existing imports remain readable; sync a repository before asking for a new tour or chat response.

Markdown supports sanitized HTML, including `details`/`summary`, tables, images, headings, lists, and inline formatting. The allowlist follows [GitHub's html-pipeline sanitizer](https://github.com/gjtorikian/html-pipeline/blob/main/lib/html_pipeline/sanitization_filter.rb); scripts, event handlers, and unsafe URLs are removed. Unlike GitHub's hidden comments, HTML comments appear as muted expandable blocks so review context remains accessible. Fenced code stays literal. Svelte and HTML files receive syntax highlighting.

Approval shields on cards and PR pages show current approvals from the last sync, with a **You** marker for the signed-in reviewer. Later change requests or dismissals replace earlier approvals from the same reviewer; comment-only reviews do not revoke approvals. Check icons have accessible names and tooltips, and link to check details when available. PR discussion starts collapsed.

Use the eye-slash **Ignore** button beside Pin on a PR card or in the PR header to move it to the **Ignored** section at the bottom of the dashboard. This local, repository-scoped preference survives refresh and sync. Ignored PRs stay out of regular and pinned groups; **Unignore** restores their normal placement and retains any saved pin and review notes.
