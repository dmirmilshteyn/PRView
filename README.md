...
- Below Info, switch between **Code** and **Tour**. Opening Tour asks Luna to build a guided review route: before/after behavior, component interactions, prioritized stops with source excerpts, specific review questions, test scenarios, and explicit coverage gaps. The left sidebar switches from file search to tour sections. **Open in Code** jumps to the cited snapshot and lines; returning to Tour resumes your section. Generation continues while you read Code, and a loading screen shows its progress.
