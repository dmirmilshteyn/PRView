export function validateChatAttachments(value) {
  if (!Array.isArray(value) || value.length > 20 || JSON.stringify(value).length > 80000) {
    throw new Error("Attach up to 20 code selections or comments totaling at most 80,000 characters");
  }
  return value.map((item) => {
    if (!item || typeof item.id !== "string" || typeof item.filePath !== "string" || !item.filePath || typeof item.body !== "string" || typeof item.revision !== "string") {
      throw new Error("Each attachment needs a file, revision, and comment text (which may be empty)");
    }
