...
export function createChatServiceWithRuntime(store, runner, active) {
  ...
  async function restart(key, requestId) {
    if (typeof requestId !== "string" || !/^[a-f0-9-]{36}$/i.test(requestId)) {
      throw new Error("A restart request ID is required");
    }
    return store.update(key, async (current) => {
      if (current.restartRequestId === requestId) {
        return current;
      }
      if (active.has(key)) {
        throw new Error("Luna is responding. Wait for it to finish before restarting.");
      }
      await store.archive(key, current, requestId);
      return { sessionId: null, messages: [], status: "idle", error: null, activity: null, contextRevision: null, restartRequestId: requestId };
    });
  }
  return { get, send, restart };
}
