...
export function createChatService(store, runner) {
  ...
  async function send(key, message, requestId, context, revision, cwd) {
    ...
    const state = await store.update(key, (current) => {
      ...
      active.add(key);
      started = true;
      return { ...current, status: "running", error: null, activity: "Reviewing…", messages: [...current.messages, { id: requestId, role: "user", text: message.trim(), createdAt: new Date().toISOString() }] };
    }).catch((error) => {
      if (started) {
        active.delete(key);
      }
