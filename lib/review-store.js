...
export async function readReview(root, repository, number) {
  ...
  try {
    // Older saved reviews predate fields such as reviews and reviewDrafts.
    // Fill missing fields at the storage boundary for every consumer.
    return { ...emptyReview(), ...JSON.parse(await readFile(filePath, "utf8")) };
  } catch (error) {
...
export function validateOperation(operation) {
  ...
  const validators = {
    cancelReview: () => validText(operation.id),
    pin: () => typeof operation.value === "boolean",
    ignore: () => typeof operation.value === "boolean",
...
export async function updateReview(root, repository, number, operation) {
  ...
  return modifyReview(root, repository, number, (current) => {
...
export async function updateReviewSubmission(root, repository, number, id, submission) {
  return modifyReview(root, repository, number, (current) => {
...
async function modifyReview(root, repository, number, transform) {
