import { tourPrompt, tourRepairPrompt, validateTour } from "./tour.js";
...
export function createTourService(store, runner, cwd) {
  ...
  async function generate(key, input, retry) {
    ...
    async function run() {
      try {
        ...
        let sessionId = null;
        async function requestTour(prompt) {
          const messages = [];
          await runner({ sessionId, cwd, repository: input.repository, prompt, onEvent: async (event) => {
            if (event.type === "thread.started") {
              sessionId = event.thread_id;
              await store.update(key, (current) => ({ ...current, sessionId }));
            }
          ...
          // Commentary may precede the complete structured response.
          return messages.at(-1) ?? "";
        }
        const response = await requestTour(tourPrompt(input, contextPath));
        let tour;
        try {
          tour = validateTour(response, input);
        } catch (error) {
          if (error.code !== "TOUR_REFERENCE_INVALID") {
            throw error;
          }
          await store.update(key, (current) => ({ ...current, activity: "Correcting code references against the snapshot…" }));
          const correction = tourRepairPrompt(error.message, response);
          const repaired = await requestTour(sessionId ? correction : `${tourPrompt(input, contextPath)}\n\n${correction}`);
          tour = validateTour(repaired, input);
        }
  ...
  async function markReviewed(key, section, reviewed) {
    ...
    return store.update(key, (current) => {
      ...
      const sections = ["overview", ...current.tour.steps.map((step) => `step-${step.id}`), ...(current.tour.mechanical?.length ? ["mechanical"] : [])];
