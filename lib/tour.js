...
{"title":"Review route title","overview":"Concise intent and before/after behavior in Markdown","flow":"How the changed components interact; mention file paths","steps":[{"title":"Stop title","risk":"high|medium|low","why":"Why to review this now and its impact","explanation":"Evidence-grounded walkthrough in Markdown","references":[{"path":"changed/file","side":"RIGHT","start":1,"end":12}],"questions":["Specific invariant or failure case to inspect"]}],"existingTests":["Tests observed in supplied code and what they cover"],"suggestedTests":["Concrete missing test or manual scenario and expected behavior"],"notCovered":[{"path":"changed/file","reason":"Why outside the stops"}],"limitations":["Missing source, uncertainty, or evidence boundary"]}
Do not invent existing tests. Empty arrays are valid except steps, references, and questions. Keep each field focused and readable.
...
export function validateTour(raw, input) {
  ...
  if (!Array.isArray(value.steps) || value.steps.length < 1 || value.steps.length > 24) {
  ...
  return { title: text(value.title, "title"), overview: text(value.overview, "overview"), flow: text(value.flow, "flow"), steps, existingTests: texts(value.existingTests, "existing tests"), suggestedTests: texts(value.suggestedTests, "suggested tests"), limitations: texts(value.limitations, "limitations"), notCovered, fileCount: files.size, coveredCount: covered.size };
}
